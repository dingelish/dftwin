...
//#include <asm/hvm/libdasm.h>
//#include <asm/hvm/opcode_tables.h>
//#include <xen/lib.h>
...
int get_operand(PINST inst, int oflags, PINSTRUCTION instruction,
	...
	int reg      = LIB_REG_NOP;
	int basereg  = LIB_REG_NOP;
	int indexreg = LIB_REG_NOP;
	int dispbytes = 0;
	enum Mode pmode;
	...
	op->reg      = LIB_REG_NOP;
	op->basereg  = LIB_REG_NOP;
	op->indexreg = LIB_REG_NOP;
	...
	if (inst->modrm) {
		...
		if (MASK_MODRM_MOD(*addr) == 0) {
			if ((pmode == MODE_32) && (MASK_MODRM_RM(*addr) == LIB_REG_EBP))
				dispbytes = 4;
			if ((pmode == MODE_16) && (MASK_MODRM_RM(*addr) == LIB_REG_ESI))
				dispbytes = 2;
		...
		// 32-bit mode
		if (pmode == MODE_32) {
			if ((MASK_MODRM_RM(*addr) == LIB_REG_ESP) &&
				...
				if ((basereg == LIB_REG_EBP) && !MASK_MODRM_MOD(*addr)) {
					basereg = LIB_REG_NOP;
						dispbytes = 4;
				}
				if (indexreg == LIB_REG_ESP)
					indexreg = LIB_REG_NOP;
			} else {
				if (!MASK_MODRM_MOD(*addr) && (MASK_MODRM_RM(*addr) == LIB_REG_EBP))
					basereg = LIB_REG_NOP;
				else
		...
		// 16-bit
			...
			switch (MASK_MODRM_RM(*addr)) {
				case 0:
					basereg  = LIB_REG_EBX;
					indexreg = LIB_REG_ESI;
					break;
				case 1:
					basereg  = LIB_REG_EBX;
					indexreg = LIB_REG_EDI;
					break;
				case 2:
					basereg  = LIB_REG_EBP;
					indexreg = LIB_REG_ESI;
...
skip_rest:
		...
		case AM_E:
			...
			if ((basereg != LIB_REG_NOP) && (MASK_MODRM_MOD(*addr) == 3)) {
				op->type = OPERAND_TYPE_REGISTER;
				op->reg  = basereg;
			}
			break;
...
int get_operand_string(INSTRUCTION *inst, OPERAND *op,
	...
	if (op->type == OPERAND_TYPE_REGISTER) {
		...
		switch (MASK_AM(op->flags)) {
			case AM_REG:
				if (MASK_FLAGS(op->flags) == F_r)
					regtype = LIB_REG_SEGMENT;
				else if (MASK_FLAGS(op->flags) == F_f)
					regtype = LIB_REG_FPU;
				else
					regtype = LIB_REG_GEN_DWORD;
				break;
			case AM_E:
			case AM_G:
			case AM_R:
			...
			// control register encoded in MODRM
			case AM_C:
				regtype = LIB_REG_CONTROL;
				break;
			// debug register encoded in MODRM
			case AM_D:
				regtype = LIB_REG_DEBUG;
				break;
			// Segment register encoded in MODRM
			case AM_S:
				regtype = LIB_REG_SEGMENT;
		...
		if (regtype == LIB_REG_GEN_DWORD) {
			 switch (MASK_OT(op->flags)) {
				case OT_b:
					snprintf(string + strlen(string), length - strlen(string),
						"%s", reg_table[LIB_REG_GEN_BYTE][op->reg]);
                                        break;
				case OT_v:
					snprintf(string + strlen(string), length - strlen(string),
						"%s", (mode == MODE_32) ?
						reg_table[LIB_REG_GEN_DWORD][op->reg] :
						reg_table[LIB_REG_GEN_WORD][op->reg]);
                                        break;
				case OT_w:
					snprintf(string + strlen(string), length - strlen(string),
						"%s", reg_table[LIB_REG_GEN_WORD][op->reg]);
					break;
				case OT_d:
					snprintf(string + strlen(string), length - strlen(string),
						"%s", reg_table[LIB_REG_GEN_DWORD][op->reg]);
					break;
			}
		} else
			snprintf(string + strlen(string), length - strlen(string),
		...
		if (MASK_PREFIX_G2(inst->flags))
			snprintf(string + strlen(string),
				...
				reg_table[LIB_REG_SEGMENT][(MASK_PREFIX_G2(inst->flags)) - 1]);
		...
		if (op->basereg != LIB_REG_NOP) {
			snprintf(string + strlen(string), length - strlen(string),
				"%s%s", (format == FORMAT_ATT) ? "%" : "",
				(mode == MODE_32) ?
				reg_table[LIB_REG_GEN_DWORD][op->basereg] :
				reg_table[LIB_REG_GEN_WORD][op->basereg]);
		}
		// Index register
		if (op->indexreg != LIB_REG_NOP) {
			if (op->basereg != LIB_REG_NOP)
				snprintf(string + strlen(string), length - strlen(string),
					"%s%s", (format == FORMAT_ATT) ? ",%" : "+",
					(mode == MODE_32) ?
					reg_table[LIB_REG_GEN_DWORD][op->indexreg] :
					reg_table[LIB_REG_GEN_WORD][op->indexreg]);
			else
				snprintf(string + strlen(string), length - strlen(string),
					"%s%s", (format == FORMAT_ATT) ? "%" : "",
					(mode == MODE_32) ?
		...
		if (inst->dispbytes && (format != FORMAT_ATT)) {
			if ((op->basereg != LIB_REG_NOP) || (op->indexreg != LIB_REG_NOP)) {
...
int get_instruction(PINSTRUCTION inst, BYTE *addr, enum Mode mode) {
	...
	inst->type =  (Instruction) MASK_TYPE_VALUE(ptr->type);
...
int get_mnemonic_string(INSTRUCTION *inst, enum Format format, char *string, int length) {
	...
	if (MASK_PREFIX_G2(inst->flags) &&
		...
		if (inst->type == INSTRUCTION_TYPE_JMPC)
			snprintf(string + strlen(string), length - strlen(string),
				"%s ", reg_table[LIB_REG_BRANCH][(MASK_PREFIX_G2(inst->flags)) - 1]);
		// Segment override for others
		else
			snprintf(string + strlen(string), length - strlen(string),
				"%s ", reg_table[LIB_REG_SEGMENT][(MASK_PREFIX_G2(inst->flags)) - 1]);
	}
...
int get_operand_value(INSTRUCTION *inst, OPERAND *op, struct cpu_user_regs *regs, struct allop *aop, int *now) {
...
//						"%s", reg_table[LIB_REG_GEN_BYTE][op->reg]);
					aop->in[*now] = *op;
					aop->in_value[*now] = read_value(0, reg_no_table[LIB_REG_GEN_BYTE][op->reg], 0, regs);
					*now ++;
                    break;
				case OT_v:
//					snprintf(string + strlen(string), length - strlen(string),
...
//						reg_table[LIB_REG_GEN_WORD][op->reg]);
					aop->in[*now] = *op;
					aop->in_value[*now] = read_value(0, reg_no_table[LIB_REG_GEN_DWORD][op->reg], 0, regs);
					*now ++;
                    break;
				case OT_w:
//					snprintf(string + strlen(string), length - strlen(string),
//						"%s", reg_table[LIB_REG_GEN_WORD][op->reg]);
					aop->in[*now] = *op;
					aop->in_value[*now] = read_value(0, reg_no_table[LIB_REG_GEN_WORD][op->reg], 0, regs);
					*now ++;
					break;
				case OT_d:
//					snprintf(string + strlen(string), length - strlen(string),
...
//				"%s%s:", (format == FORMAT_ATT) ? "%" : "",
//				reg_table[LIB_REG_SEGMENT][(MASK_PREFIX_G2(inst->flags)) - 1]);
			sum_addr = read_value(0, reg_no_table[LIB_REG_SEGMENT][(MASK_PREFIX_G2(inst->flags)) - 1], 0, regs);
		}
		// Some ATT stuff we need to check at this point
...
//				reg_table[LIB_REG_GEN_DWORD][op->basereg] :
//				reg_table[LIB_REG_GEN_WORD][op->basereg]);
			sum_addr += read_value(0, reg_no_table[LIB_REG_GEN_DWORD][op->basereg], 0, regs);
		}
		// Index register
		if (op->indexreg != LIB_REG_NOP) {
			unsigned int index_reg = 0;
			if (op->basereg != LIB_REG_NOP){
				//snprintf(string + strlen(string), length - strlen(string),
				//	"%s%s", (format == FORMAT_ATT) ? ",%" : "+",
				//	(mode == MODE_32) ?
				//	reg_table[LIB_REG_GEN_DWORD][op->indexreg] :
				//	reg_table[LIB_REG_GEN_WORD][op->indexreg]);
				index_reg = read_value(0, reg_no_table[LIB_REG_GEN_DWORD][op->basereg], 0, regs);
			}
			else{
				//snprintf(string + strlen(string), length - strlen(string),
				//	"%s%s", (format == FORMAT_ATT) ? "%" : "",
...
int get_output_value(INSTRUCTION *inst, struct cpu_user_regs *regs, struct allop *aop){
	switch (inst->opcode){

	}
	return 0;
}
int get_operands_value(INSTRUCTION *inst, DWORD offset,	struct allop *aop, struct cpu_user_regs *regs) {
	int in = 0;

	if (inst->op1.type != OPERAND_TYPE_NONE){
		get_operand_value(inst, &inst->op1, regs, aop, &in);
	}
	if (inst->op2.type != OPERAND_TYPE_NONE) {
		//snprintf(string + strlen(string), length - strlen(string), ",");
		//get_operand_string(inst, &inst->op2, format, offset,
	...
	get_output_value(inst, regs, aop);
	return 1;
}
unsigned int read_value(int ismem, int reg_id, int mem_address, struct cpu_user_regs *regs){
	if(ismem){
		//TODO: read from mem
		//      copy_from_guest_virt
	}
	else{
...
//	{ "??",   "(bnt)","??",   "(bt)", "??",   "??",   "??",   "??"   },
//};
//const unsigned short reg_no_table[11][8] = {
//	{ 0,   1,  2,   3,  4,    5,   6, 7 },
...
//};
		switch(reg_id){
			...
			case 20:
				return (regs->eax & 0x0000FF00 ) >> 8;
			case 21:
				return (regs->ecx & 0x0000FF00 ) >> 8;
			case 22:
				return (regs->edx & 0x0000FF00 ) >> 8;
			case 23:
				return (regs->ebx & 0x0000FF00 ) >> 8;
			//	{ "es",   "cs",   "ss",   "ds",   "fs",   "gs",   "??",   "??"   },
			//	{ 24, 25, 26,  27,  28,  29,  30, 31},
			case 24:
				return regs->es;
